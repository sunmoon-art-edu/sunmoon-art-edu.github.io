...
ngphap={}
gf=f'{KIND}{LV}-nguphap.js'
if os.path.exists(gf):
    o=subprocess.run(['node','-e',"const fs=require('fs'),vm=require('vm');const c=vm.createContext({});vm.runInContext(fs.readFileSync(process.argv[1],'utf8')+';globalThis.__G=NGUPHAP;',c);process.stdout.write(JSON.stringify(c.__G))",gf],capture_output=True,text=True)
    if o.returncode==0: ngphap=json.loads(o.stdout)
    else: print('!! lỗi', gf, o.stderr[:200])
s=s.replace('/*NGUPHAP*/{}',json.dumps(ngphap,ensure_ascii=False,separators=(',',':')))
